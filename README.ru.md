...
1. **Теплоперенос** — обновление $\theta$, штрафного слагаемого и плавучести.
...
[Физическую модель](#физическая-модель)). Формы сглаживания для функции Хевисайда (ступеньки) и
...
противопоточной аппроксимации до схемы с отложенной коррекцией. Уравнение энергии
...
становится большим в твёрдой фазе, обнуляя там скорость; его форма заменяема
...
| `octadecane/` | Плавление n-октадекана в разнонагретой полости  |
...
| `icicle/` | Таяние ледяного стержня                         |
| `crevasse/` | Замерзание трещины (ледниковой)                 |
