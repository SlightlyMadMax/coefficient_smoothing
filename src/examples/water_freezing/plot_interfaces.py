...
AXES = {
    ...
    "penalty": ("penalty_C", lambda v: rf"$C = 10^{{{int(round(np.log10(v)))}}}$ s$^{{-1}}$"),
}
...
def parse_args(argv=None) -> argparse.Namespace:
    ...
    p.add_argument(
        "--grids", type=int, nargs="*", default=None,
...
def load_runs(data_dir: Path) -> list[dict]:
    ...
    for sub in sorted(data_dir.iterdir()):
        ...
        checkpoints = sorted(sub.glob("checkpoint_*.npz"),
...
def _legend(ax, n_entries: int, over_image: bool):
    """Legend that stays compact as the number of curves grows."""
    kw = dict(loc="upper left", ncol=2 if n_entries > 4 else 1,
              columnspacing=1.0, handlelength=1.4)
    if over_image:
        return ax.legend(frameon=True, framealpha=0.85, facecolor="white",
                         edgecolor="none", **kw)
    return ax.legend(frameon=False, **kw)


def _add_spread_inset(ax, curves, height: float, half_height: float) -> None:
    """
    ...
    band = (yy >= lo) & (yy <= hi)
    x_lo = np.nanmin(stack[:, band])
    x_hi = np.nanmax(stack[:, band])
    pad = max(0.15 * (x_hi - x_lo), 2e-5)

    # The left half of the domain is empty at the comparison time, so the inset goes
    # there; connectors run from its right edge to the magnified band on the right.
    # The legend occupies the upper left and grows with the number of curves, so drop
    # the inset lower once there are enough entries for the two to collide.
    loc = "lower left" if len(curves) > 4 else "center left"
    axins = inset_axes(ax, width="38%", height="26%", loc=loc, borderpad=0.8)
    ...
    # Scale bar: the reader needs to know what the magnified spread is worth
...
def _select(runs: list[dict], axis: str,
...
LABELLERS = {
    ...
    "eps_T": lambda v: rf"$arepsilon_T={v:g}$ K",
    "eps_flow": lambda v: rf"$arepsilon_\mathrm{{flow}}={v:g}$ K",
...
def plot_explicit(args: argparse.Namespace) -> bool:
    ...
    for name in args.runs:
        ...
        checkpoints = sorted(d.glob("checkpoint_*.npz"),
                             key=lambda f: int(f.stem.split("_")[1]))
        if not summary.exists() or not checkpoints:
    ...
    _legend(ax, len(curves), over_image)
    if not args.no_inset and len(curves) > 1:
        _add_spread_inset(ax, curves, height, args.inset_half_height)
    ...
    for r, lab in zip(sel, labels):
        print(f"      {lab:<44} t={r['t_saved']:.0f} s  "
...
def plot_axis(runs: list[dict], axis: str, args: argparse.Namespace) -> bool:
    ...
    if args.piv_background is not None:
        ax.imshow(plt.imread(args.piv_background),
                  extent=[0.0, width0, 0.0, height0])
        colors = plt.cm.autumn(np.linspace(0.0, 0.8, len(sel)))
    else:
        colors = plt.cm.viridis(np.linspace(0.0, 0.85, len(sel)))
    curves = []
    ...
    ax.set_xlim(0.0, width)
    ax.set_ylim(0.0, height)
    _legend(ax, len(sel), args.piv_background is not None)
    ...
    for r in sel:
        print(f"      {labeller(r[field]):<28} t={r['t_saved']:.0f} s  "
              f"mean x_int = {r['mean_interface_x']:.6f} m  "
              f"ice = {r['ice_fraction']:.5f}")
    if not args.show:
        plt.close(fig)
...
def plot_deviation(sel: list[dict], axis: str, args: argparse.Namespace) -> bool:
    ...
    ax.annotate(rf"$\pm\,\Delta x$ ({cell:.0f} $\mu$m)", xy=(cell, yy.max()),
                xytext=(3, -8), textcoords="offset points",
                fontsize=7, color="0.45", ha="left", va="top")

    ax.set_xlabel(r"$x - x_{\mathrm{ref}}$, $\mu$m")
    ax.set_ylabel(r"$y$, m")
    ax.set_ylim(yy.min(), yy.max())
    ax.legend(loc="best", frameon=False,
              title=f"vs {labeller(ref[field])}", title_fontsize=8)
...
def plot_eps_split(runs: list[dict], args: argparse.Namespace) -> bool:
    ...
    e0 = Counter(r["eps_T"] for r in runs
                 if r["eps_T"] == r["eps_flow"]).most_common(1)[0][0]
    sel = [r for r in runs
    ...
    for r, col in zip(uniq, colors):
        ...
        if xb.size == 0:
            continue
        lbl = (rf"$\varepsilon = \varepsilon_f = {r['eps_T']:g}$ K"
    ...
    fig.savefig(out, dpi=300, bbox_inches="tight")
    print(f"  wrote {out}")
    print(f"    axis 'eps_split': {len(uniq)} curve(s); "
          f"held fixed: " + ", ".join(f"{f}={baseline[f]:g}" for f in fixed))
    for r in uniq:
        print(f"      eps_T={r['eps_T']:<5g} eps_flow={r['eps_flow']:<5g} "
              f"mean x_int = {r['mean_interface_x']:.6f} m  "
              f"ice = {r['ice_fraction']:.5f}")
    if not args.show:
        plt.close(fig)
...
def main(argv=None) -> None:
    ...
    if not runs:
        raise SystemExit(f"No completed runs with a checkpoint found in {args.data_dir}")
