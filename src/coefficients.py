...
def delta_function(T_j_i: float, _delta: float):
    ...
    """
    return math.exp(-(T_j_i - cfg.T_0)*(T_j_i - cfg.T_0)/(2.0*_delta*_delta)) / ((2.0*math.pi)**0.5 * _delta)
...
def c_smoothed(T_j_i: float, _delta: float):
    ...
    return cfg.C_ICE_VOL + (cfg.C_WATER_VOL - cfg.C_ICE_VOL) * (1.0 + math.erf((T_j_i - cfg.T_0)/(2**0.5*_delta))) * 0.5 + \
...
def k_smoothed(T_j_i: float, _delta: float):
    ...
    return cfg.K_ICE + (cfg.K_WATER - cfg.K_ICE) * (1.0 + math.erf((T_j_i - cfg.T_0)/(2**0.5*_delta))) * 0.5
...
def c_simple(T_j_i: float, _delta: float):
    if abs(T_j_i - cfg.T_0) <= _delta:
        return 0.5 * (cfg.C_WATER_VOL + cfg.C_ICE_VOL) + cfg.L_VOL * delta_const(T_j_i, _delta)
...
def k_simple(T_j_i: float, _delta: float):
    if abs(T_j_i - cfg.T_0) <= _delta:
        return cfg.K_WATER + 0.5 * (cfg.K_WATER - cfg.K_ICE) * (T_j_i - cfg.T_0 - _delta) / _delta
    elif T_j_i < cfg.T_0 - _delta:
        return cfg.K_ICE
    else:
        return cfg.K_WATER
